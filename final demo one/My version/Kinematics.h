...
MatrixXf inverseKinematics(float gPos[]) //almost done
{
	...
	if (noSolution > 1)
	{
		cout << "there is no solution as b equals " << noSolution << endl;
		return solVector;
	}
	...
	{
		cout << "Out of joint 2 lim as theta2 equals  " << toDegree(theta2) << endl;
	...
	{
		cout << "Out of joint 1 lim as theta1 equals" << toDegree(theta1) << endl;
	...
	{
		cout << "there is only one solution theta1_2 " << toDegree(theta1_2) << endl;
	...
	cout << "theta3 = " << toDegree(-theta3) << " and theta 3_2 = " << toDegree(-theta3_2) << endl;

	if (theta3 < joint4Lim[0] || theta3 > joint4Lim[1])
	{
		cout << "Out of joint 4 lim as theta3 equals " << theta3 << endl;
		return solVector;
	}
	...
	{
		cout << "Out of joint 3 lim as d equals " << d << endl;
		return solVector;
	}
...
void moveToCoordinates() //WIP
{
	...
	while (1)
	{
		...
		if (c != ESC)
		{
			...
			float q[4];
			cout << "please enter the valuse of X: ";
			cin >> q[0];
			cout << "please enter the valuse of Y: ";
			cin >> q[1];
			cout << "please enter the valuse of Z: ";
			cin >> q[2];
			cout << "please enter the valuse of Phi in Degrees: ";
			cin >> q[3];
			...
			if (solVector(2, 0) != 0)
			{
				...
				else if (ch == '3')
				{
					...
					dif1_1 = abs(S_1[0] - current[0]); while (dif1_1 > 360) { dif1_1 = dif1_1 - 360; }
					dif1_2 = abs(S_2[0] - current[0]); while (dif1_2 > 360) { dif1_2 = dif1_2 - 360; }
					dif2_1 = abs(S_1[1] - current[1]); while (dif2_1 > 360) { dif2_1 = dif2_1 - 360; }
					dif2_2 = abs(S_1[1] - current[1]); while (dif2_2 > 360) { dif2_2 = dif2_2 - 360; }

					if ((L1 * toRad(dif1_1) + L2 * toRad(dif2_1)) > (L1 * toRad(dif1_2) + L2 * toRad(dif2_2)) || solVector(2, 1) == 0)
					{
						MoveToConfiguration(S_1);
						for (int i = 0; i < 4; i++) { current[i] = S_1[i]; }
					}
